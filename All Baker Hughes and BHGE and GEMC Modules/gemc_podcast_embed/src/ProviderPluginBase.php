...
abstract class ProviderPluginBase extends PluginBase implements ProviderPluginInterface, ContainerFactoryPluginInterface {
   ...
   * @var string
   */
  protected $thumbsDirectory = 'public://podcast_thumbnails';
  ...
  public function __construct($configuration, $plugin_id, $plugin_definition, ClientInterface $http_client) {
    ...
    if (!static::isApplicable($configuration['input'])) {
      throw new \Exception('Tried to create a podcast provider plugin with invalid input.');
    }
    $this->input = $configuration['input'];
    $this->podcastId = $this->getIdFromInput($configuration['input']);
   ...
   *   The podcast ID.
   */
  protected function getPodcastId() {
  ...
  public function getLocalThumbnailUri() {
    return $this->thumbsDirectory . '/' . $this->getPodcastId() . '.jpg';
  }
  ...
  public function getName() {
    return $this->t('@provider Podcast (@id)', ['@provider' => $this->getPluginDefinition()['title'], '@id' => $this->getPodcastId()]);
  }
